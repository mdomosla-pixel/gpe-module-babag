...
describe 'babag::smtp class' do

  context 'smtp' do
    # Using puppet_apply as a helper
    it 'should install smtp' do
    ...
    describe service('babag-smtp') do
      it {should be_enabled}
      it {should be_running}
    end

    describe package('babag-smtp-service') do
      it {should be_installed}
    end

    describe file('/etc/default/babag-smtp') do
      ...
      its(:content) {should match /^NREPL__PORT="1235"/}
      ...
      its(:content) {should match /^SMTP__PORT="2026"/}

      its(:content) {should match /^SMS__USER="smtp"/}
      its(:content) {should match /^SMS__FROM="circlek"/}
      its(:content) {should match /^SMS__PROVIDER="linkmobility"/}

      its(:content) {should match /^ADMIN__ENABLED="true"/}
      its(:content) {should match /^ADMIN__BIND_ADDRESS="0.0.0.0"/}
      its(:content) {should match /^ADMIN__PORT="9092"/}
